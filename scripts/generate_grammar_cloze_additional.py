"""
Grammar Cloze Generation Script using Vertex AI - Additional 401 passages
Generates comprehensive grammar cloze passages with mixed topics.

Usage: python generate_grammar_cloze_additional.py
"""
...
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "../src/data/grammar_cloze_additional.json")
...
def main():
    print("=" * 60)
    print("GRAMMAR CLOZE GENERATION - ADDITIONAL 401 PASSAGES")
    print("Mixed Topics | 5 Paragraphs × 4 Sentences × 2 Questions")
    print("=" * 60)
    
    # Load existing passages to get the correct starting ID
    try:
        ...
        start_id = 1
        print("\nNo existing passages found, starting from ID 1")
    
    model = init_vertex_ai()
    new_passages = []
    
    # Target: 401 passages distributed across difficulties
    targets = {
        3: 80,  # Primary 3-4
        4: 81,  # Primary 5-6 (PSLE)
        5: 80,  # Secondary 1-2
        6: 80,  # Secondary 3-4 (O-Level)
        7: 80   # JC (A-Level)
    }
    ...
    for difficulty, count in targets.items():
        ...
        for i in range(count):
            print(f"  Generating passage {i+1}/{count}... (ID: {passage_id})", end=" ", flush=True)
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump(new_passages, f, indent=2, ensure_ascii=False)
    
    print(f"\n{'=' * 60}")
    print(f"COMPLETE: {len(new_passages)} new passages generated")
    print(f"Saved to: {OUTPUT_FILE}")
    print(f"Next step: Merge with {EXISTING_FILE}")
    print("=" * 60)
